...
namespace TypeReferences.Editors
{
    /// <summary>
    ...
    public sealed class ClassTypeReferencePropertyDrawer : PropertyDrawer
    {
        ...
        ///     </para>
        ///     <para>
        ///         Since filtering makes extensive use of <see cref="ICollection{Type}.Contains" />
        ...
        private static List<Type> GetFilteredTypes(ClassTypeConstraintAttribute filter)
        {
            ...
            ICollection<Type> excludedTypes =
                ExcludedTypeCollectionGetter != null ? ExcludedTypeCollectionGetter() : null;
            ...
            ICollection<Type> excludedTypes, List<Type> output)
        {
            foreach (Type type in assembly.GetTypes())
            {
                ...
                if (filter is ClassExtendsAttribute or ClassImplementsAttribute)
                {
                    if (!filter.AllowAbstract && !type.IsClass) continue;
                }

                if (filter is InterfaceOnlyAttribute)
                {
                ...
                if (excludedTypes != null && excludedTypes.Contains(type)) continue;
